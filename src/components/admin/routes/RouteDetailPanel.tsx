...
export function RouteDetailPanel({ route, outletId }: RouteDetailPanelProps) {
  ...
  const toggleSelect = useCallback((orderId: string, source: 'daily' | 'addon') => {
    if (source === 'daily') {
  ...
  const toggleSelectAll = useCallback(() => {
    if (selectedDaily.length + selectedAddon.length === orders.length) {
      setSelectedDaily([]);
      setSelectedAddon([]);
    } else {
  ...
  }, [orders, selectedDaily, selectedAddon]);
              ...
              <TableRow>
                <TableHead className="w-10">
                  <Checkbox
                    checked={
                      orders.length > 0 &&
                      selectedDaily.length + selectedAddon.length === orders.length
                    }
