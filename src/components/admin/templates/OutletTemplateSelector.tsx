...
interface OutletTemplateSelectorProps {
  onOutletChange: (outletId: string) => void;
}

export function OutletTemplateSelector({ onOutletChange }: OutletTemplateSelectorProps) {
  ...
  // Auto-select assigned outlet for non-SUPER_ADMIN roles
  useEffect(() => {
  ...
  // For non-SUPER_ADMIN, don't render the selector — outlet is auto-selected
  if (!isSuperAdmin) {
    if (!selectedOutletId) {
