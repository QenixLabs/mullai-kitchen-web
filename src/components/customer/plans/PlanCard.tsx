...
export function PlanCard({
  ...
  if (variant === "compact") {
    return (
      <article
        className={cn(
          "group relative flex w-72 flex-col overflow-hidden rounded-lg bg-white shrink-0",
          "shadow-md",
      ...
      >
        {/* Selection indicator */}
        {isSelected && (
          <div className="absolute right-2 top-2 z-20 flex h-6 w-6 items-center justify-center rounded-lg bg-primary shadow-lg">
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <div className="mb-4 flex gap-1.5 overflow-x-auto pb-1 [-webkit-overflow-scrolling:touch] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {plan.meals_included.map((meal) => (
              <span
                key={`${plan._id}-${meal}`}
                className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-gray-100 px-2.5 py-1 text-xs text-gray-600"
              >
          ...
          <Button
            type="button"
            className={cn(
              "mt-auto h-10 rounded-lg bg-primary font-semibold text-primary-foreground shadow-md transition-all duration-300",
  ...
  return (
    <article
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-lg bg-white transition-all duration-500 ease-out",
        "shadow-md",
    ...
    >
      {/* Selection indicator */}
      {isSelected && (
        <div className="absolute right-3 top-3 z-20 flex h-8 w-8 items-center justify-center rounded-lg bg-primary shadow-lg">
      ...
      <div className="relative h-44 overflow-hidden sm:h-52 lg:h-48">
        ...
        <div className="absolute bottom-3 left-3 z-10 flex items-center gap-2">
          <div className={cn(
            "flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium backdrop-blur-md",
            isWeekly
        ...
        <div className="absolute bottom-3 right-3 z-10">
          <div className="flex flex-col items-end rounded-lg bg-black/30 px-3 py-2 backdrop-blur-sm">
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
          ...
          {plan.meals_included.length > 0 ? (
            <ul className="grid grid-cols-2 gap-1.5 sm:gap-2" aria-label={`Meals in ${plan.name}`}>
              {plan.meals_included.map((meal) => (
                ...
                >
                  <span className="flex h-4 w-4 items-center justify-center rounded-lg bg-primary/10 sm:h-5 sm:w-5">
        ...
        <div className="flex flex-col gap-2 sm:gap-2.5">
          <Button
            type="button"
            className={cn(
              "relative h-10 overflow-hidden rounded-lg bg-primary font-semibold text-primary-foreground shadow-md transition-all duration-300 sm:h-11",
              "hover:bg-primary/90 hover:shadow-lg hover:shadow-primary/10",
              "active:scale-[0.98]",
          ...
          <Button
            ...
            variant="outline"
            className={cn(
              "h-9 rounded-lg font-medium transition-all duration-300 sm:h-10",
      ...
      <div
        className={cn(
          "absolute -right-12 -top-12 h-24 w-24 rounded-lg bg-primary/10 blur-2xl transition-opacity duration-500",
