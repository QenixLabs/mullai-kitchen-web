...
export function WeeklyMenuPreview({ params }: WeeklyMenuPreviewProps) {
  ...
  if (error) {
    return (
      <div className="p-6 rounded-lg border-2 border-red-50 text-center">
  ...
  if (!data || data.menu.length === 0) {
    return (
      <div className="p-12 rounded-lg border border-dashed border-gray-200 bg-gray-50/50 text-center">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {menuPreview.map((item, idx) => (
          <div
            key={item.day}
            className="rounded-lg border border-slate-100 overflow-hidden bg-white shadow-sm hover:shadow-md transition-all duration-300 group"
          >
